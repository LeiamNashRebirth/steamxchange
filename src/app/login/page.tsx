    ...
    <img src="/logo.png" alt="leiamnash"/>
        ...
        <>
                ...
                <div className="mt-6 space-y-4">
                  <input type="text" className="w-full p-3 rounded border bg-gray-900 border-gray-600 placeholder-gray-400" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} />
                  <input type="email" className="w-full p-3 rounded bg-gray-900 border border-gray-600 placeholder-gray-400" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
                  ...
                  <button 
                    onClick={nextStep} 
                    disabled={!name || !email || !dob || !!error}
                    className={`w-full py-3 rounded-full text-lg font-bold ${name && email && dob && !error ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-500 cursor-not-allowed'}`}>
            ...
            {step === 2 && (
              <>
                ...
                <div className="mt-6 space-y-4">
                <input type="text" className="w-full p-3 rounded bg-gray-900 border border-gray-600 text-white" placeholder="Verification code" value={otp} onChange={(e) => setOtp(e.target.value)} />
  {otpError && <p className="text-red-500">{otpError}</p> }
                <button onClick={nextStep} disabled={!otp} className={`w-full py-3 rounded-full text-lg font-bold ${otp ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-500 cursor-not-allowed'}`}>
                  Next
                </button>
                </div>
              </>
            ...
            {step === 3 && (
              <>
                ...
                <div className="mt-6 space-y-4">
                <div className="relative">
                  <input type={passwordVisible ? "text" : "password"} className="w-full p-3 rounded bg-gray-900 border border-gray-600 text-white" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
                  <span className="absolute right-3 top-3 cursor-pointer" onClick={() => setPasswordVisible(!passwordVisible)}>
                    {passwordVisible ? <EyeOff size={20} /> : <Eye size={20} />}
                  </span>
                </div>
                <button onClick={nextStep} disabled={!password} className={`w-full py-3 rounded-full text-lg font-bold ${password ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-500 cursor-not-allowed'}`}>Next</button>
                </div>
              </>
            )}
            ...
            {step === 4 && (
              <>
                ...
                <div className="mt-6 space-y-4">
                <select className="w-full p-3 rounded bg-gray-900 border border-gray-600 text-white" value={gradeLevel} onChange={(e) => setGradeLevel(e.target.value)}>
                  <option value="">Select Grade Level</option>
                  <option value="11">11</option>
                  <option value="12">12</option>
                </select>
                <select className="w-full p-3 rounded bg-gray-900 border border-gray-600 text-white" value={section} onChange={(e) => setSection(e.target.value)}>
                  <option value="">Select Section</option>
                  {sections.map((sec) => <option key={sec} value={sec}>{sec}</option>)}
                </select>
                  <input type="text" className="w-full p-3 rounded border bg-gray-900 border-gray-600 placeholder-gray-400" placeholder="School ID" value={schoolID} onChange={(e) => setSchoolID(e.target.value)} />
                <button onClick={nextStep} disabled={!gradeLevel || !section || !schoolID} className={`w-full py-3 rounded-full text-lg font-bold ${gradeLevel && section && schoolID ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-500 cursor-not-allowed'}`}>Next</button>
                </div>
              </>
            )}
            ...
            {step === 5 && (
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {advisers.map((adv) => (
                    <div key={adv.id} className={`p-4 border rounded text-center cursor-pointer ${adviser?.id === adv.id ? 'border-blue-500' : ''}`} onClick={() => setAdviser(adv)}>
                      <p className="font-bold">{adv.name}</p>
                    </div>
                  ))}
                </div>
                <button onClick={nextStep} disabled={!adviser} className={`w-full py-3 rounded-full text-lg font-bold ${adviser ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-500 cursor-not-allowed'}`}>Create Account</button>
                </div>
              </>
            )}
          </>

...
const CommentSection = ({ postId, onClose }) => {
  ...
  const fetchComments = async () => {
    try {
      ...
      if (!clientUID) return;
      const allPosts = await database.getDiscussionData();
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      await database.addDiscussionComment(postId, newComment);
      setCommentText('');
      scrollToBottom();
    } catch (error) {} finally {
      setIsPosting(false);
  ...
  const handleReplySubmit = async (commentDate) => {
    ...
    try {
      ...
      await database.replyDiscussionComment(postId, commentDate, newReply);
      scrollToBottom();
    } catch (error) {} finally {

...
const Images = ({ src }: { src?: string }) => {
  ...
  return (
    <div>
      {showSkeleton && (
        <div className="w-full max-w-xs h-[500px] bg-gray-700 animate-pulse rounded-lg mb-4"></div>
      )}
      ...
      {src && (
        <img
          src={src}
          alt="LeiamNash"
          className={`rounded-lg w-full max-w-xs hover:scale-105 transition-opacity duration-500 ${imageLoaded ? "opacity-100" : "opacity-0"}`}
          onLoad={() => {
            setImageLoaded(true);
            setShowSkeleton(false);
          }}

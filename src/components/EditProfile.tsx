...
const EditProfile = ({ userData, setEditOpen }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-96 relative animate-fade-in">
        ...
        <div
          className="relative w-full h-32 bg-gray-700 rounded-lg overflow-hidden flex justify-center items-center cursor-pointer"
        ...
        <div className="flex justify-center -mt-12 relative">
          <div className="w-24 h-24 rounded-full border-4 border-gray-900 bg-gray-700 overflow-hidden flex items-center justify-center cursor-pointer" onClick={() => iconInputRef.current?.click()}>
            {previewIcon ? (
        ...
        <input type="text" placeholder="Bio" value={bio} onChange={(e) => setBio(e.target.value)} className="w-full bg-gray-700 text-white p-2 rounded-md mt-4" />

        {address ? (
          <div className="w-full bg-gray-700 text-white p-2 rounded-md mt-4 text-center">{address}</div>
        ) : (
          <button onClick={handleUpdateAddress} className="w-full bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-md mt-4 flex items-center justify-center">
            {loadingAddress ? <Loader2 className="w-5 h-5 animate-spin" /> : "Update My Address"}
          </button>
        )}

        <button onClick={handleUpdate} className="w-full bg-blue-500 hover:bg-blue-600 py-2 rounded-md flex items-center justify-center mt-6 transition">
